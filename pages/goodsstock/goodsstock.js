...
Page({
    ...
    onShow: function() {
        this.getGoodsList();
        this.getgoodscateListcateradio();
    },
    ...
    getGoodsList: function() {
        ...
        if (sorttypequery == 'default') {
            goodList = app.globalData.goodList;
            ...
            if (sortorderquery == 'desc') {
                ...
                for (i = 0; i < goodsList.length; i++) {
                    for (j = 0; j < goodsList.length; j++) {
                        if (goodsList[i].num < goodsList[j].num) { // 相邻元素两两对比
                            let t = goodsList[i];
        ...
        if (this.data.searchStatus == true) {
            ...
            for (var i = 0; i < goodList.length; i++) {
                if (goodList[i].name.indexOf(searchquery) >= 0) {
                    goodsList.push(goodList[i]);
                }
            }
            goodList = goodsList;
        }
        if (sexradiochoiceid != 'default') {
            goodsList = [];
            for (var i = 0; i < goodList.length; i++) {
                // console.log(goodList[i].cate)
                if (sexradiochoiceid == goodList[i].cate) {
                    goodsList.push(goodList[i]);
                }
            }
            goodList = goodsList;
    ...
    //获取条件筛选选项
    getgoodscateListcateradio: function() {
        var goodscateList = app.globalData.goodscateList;
        this.setData({
                goodscateList,
            })
            // console.log(this.data.goodscateList, this.data.sexradio)
    },
